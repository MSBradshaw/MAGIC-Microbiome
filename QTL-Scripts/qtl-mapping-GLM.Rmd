...
pheno <- read.table("../OTU_Data/novo_rarefied_6356.tsv", header=TRUE, sep="\t", row.names = 1, check.names=FALSE,stringsAsFactors=FALSE,comment.char="@")
...
#filter out taxes that are all na
```
...
This is also the point where you normalize as well.
Normalization is not required but it helps. The model can handel non-normalized data, with some reduction of statistical power
...
phenotype = as.numeric(unlist(pheno[,INPUT_COL]))
...
For allele coding, the dat matrix should be:

  dat = cbind(1,suppressWarnings(as.factor(x)),phenotype)

For 012 coding, the dat matrix should be:
...
lod_RIL = apply(g_data, 1, function(x){
  dat = cbind(100,suppressWarnings(as.factor(x)),phenotype)
  #dat = cbind(1,suppressWarnings(as.factor(as.numeric(x)),phenotype)
  ...
  fit0 = glm(Phenotype~a,family=poisson(),data=dat)
  fit1 = glm(Phenotype~cbind(a,Allele),family=poisson(),data=dat)
  
  res0 = resid(fit0, type = "deviance")
  res1 = resid(fit1,type = "deviance")
 rss0 = sum(res0^2)
 rss1 = sum(res1^2)
 (dim(dat)[1]/2)*(log10(rss0/rss1))
...
```{r structure of the candidate gene list}
head(candidates)
...
```{r enhance candidate list}
joined <- left_join(candidates, aggr, by = "gene")
head(joined)
```

```{r unique candidate list}
...
```{r save candidate list}
write.table(joined, paste(OUTPUT_DIR_NAME,"/CandidateGenes.txt",sep=''), row.names=F,
