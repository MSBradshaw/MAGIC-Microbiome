...
#Ilr transformation
phenotype_ilr = ilr(pheno)

#plot it to visualize the normalization
phenotype <- as.numeric(unlist(phenotype_ilr[,INPUT_COL]))
...
write.table(joined, paste(OUTPUT_DIR_NAME,"/CandidateGenes.txt",sep=''), row.names=F, col.names=F, sep="\t", quote=FALSE) 
